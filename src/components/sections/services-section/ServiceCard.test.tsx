...
describe("ServiceCard", () => {
  describe("Icon Selection Logic", () => {
    ...
    it('renders Home icon for title containing "verkauf"', () => {
      render(
        <ServiceCard
          title="Immobilien Verkauf Service"
          text="Text"
          link="/link"
        />,
      );
    ...
    it('renders Building2 icon for title containing "vermiet"', () => {
      render(
        <ServiceCard title="Wohnung Vermietung" text="Text" link="/link" />,
      );
      expect(screen.getByTestId("building2-icon")).toBeInTheDocument();
    });
    ...
    it('renders BarChart3 icon for title containing "bewert"', () => {
      render(
        <ServiceCard
          title="Immobilienbewertung Service"
          text="Text"
          link="/link"
        />,
      );
  ...
  describe("Rendering and structure", () => {
    ...
    it("uses correct icon styling classes", () => {
      render(<ServiceCard title="Verkaufen" text="Text" link="/link" />);
      expect(screen.getByTestId("home-icon")).toHaveClass(
        "h-5",
        "w-5",
        "text-primary",
      );
  ...
  describe("Edge Cases", () => {
    ...
    it('handles title with special characters and "verkauf"', () => {
      render(
        <ServiceCard title="Verkauf & Beratung" text="Text" link="/link" />,
      );
      expect(screen.getByTestId("home-icon")).toBeInTheDocument();
    });
    ...
    it("matches first keyword when multiple keywords present", () => {
      render(
        <ServiceCard title="Verkauf und Vermietung" text="Text" link="/link" />,
      );
      expect(screen.getByTestId("home-icon")).toBeInTheDocument();
    });
    ...
    it("renders correctly with very long title", () => {
      const longTitle =
        "Verkaufen von Immobilien mit professioneller Beratung und Service";
    ...
    it("renders correctly with very long text", () => {
      const longText =
        "Dies ist eine sehr lange Beschreibung die mehrere Zeilen umfassen kann und viele Details über den Service enthält";
