...
describe('LegalDocumentLayout', () => {
  it('renders title as h1', () => {
    render(<LegalDocumentLayout {...defaultProps}><p>Content</p></LegalDocumentLayout>);
  ...
  it('renders TOC anchor links with correct hrefs', () => {
    render(<LegalDocumentLayout {...defaultProps}><p>Content</p></LegalDocumentLayout>);
    expect(screen.getByRole('link', { name: /abschnitt 1/i })).toHaveAttribute('href', '#section-1');
    expect(screen.getByRole('link', { name: /abschnitt 2/i })).toHaveAttribute('href', '#section-2');
  });

  it('renders children content', () => {
    render(<LegalDocumentLayout {...defaultProps}><p>My Content</p></LegalDocumentLayout>);
...
describe('LegalSection', () => {
  it('renders title as h2', () => {
    render(<LegalSection id="test" title="Test Section"><p>Body</p></LegalSection>);
    expect(screen.getByRole('heading', { level: 2, name: 'Test Section' })).toBeInTheDocument();
  });

  it('renders description when provided', () => {
    render(<LegalSection id="test" title="Title" description="Section desc"><p>Body</p></LegalSection>);
    expect(screen.getByText('Section desc')).toBeInTheDocument();
  });

  it('does not render description paragraph when not provided', () => {
    const { container } = render(<LegalSection id="test" title="Title"><p>Body</p></LegalSection>);
    expect(container.querySelectorAll('p')).toHaveLength(1);
  });

  it('renders children', () => {
    render(<LegalSection id="test" title="Title"><p>Body text</p></LegalSection>);
    expect(screen.getByText('Body text')).toBeInTheDocument();
  });

  it('sets correct id on section element', () => {
    const { container } = render(<LegalSection id="my-section" title="Title"><p>Body</p></LegalSection>);
