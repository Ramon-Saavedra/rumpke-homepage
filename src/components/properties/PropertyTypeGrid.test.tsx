...
describe("PropertyTypeGrid", () => {
  ...
  it("renders correct links for each type", () => {
    render(<PropertyTypeGrid types={types} basePath="kauf" />);
    expect(screen.getByRole("link", { name: /häuser/i })).toHaveAttribute(
      "href",
      "/kauf/haeuser",
    );
    expect(screen.getByRole("link", { name: /wohnungen/i })).toHaveAttribute(
      "href",
      "/kauf/wohnungen",
    );
  });
  ...
  it("renders title heading when provided", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Nach Typ filtern" }),
    ).toBeInTheDocument();
  });
