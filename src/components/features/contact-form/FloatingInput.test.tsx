...
describe('FloatingInput', () => {
  ...
  it('shows error message when error prop is provided', () => {
    render(<FloatingInput id="test-input" label="Vorname" error="Pflichtfeld" />);
  ...
  it('sets aria-invalid to true when error is present', () => {
    render(<FloatingInput id="test-input" label="Vorname" error="Pflichtfeld" />);
    expect(screen.getByLabelText(/Vorname/i)).toHaveAttribute('aria-invalid', 'true');
  });

  it('sets aria-invalid to false when no error', () => {
    render(<FloatingInput id="test-input" label="Vorname" />);
    expect(screen.getByLabelText(/Vorname/i)).toHaveAttribute('aria-invalid', 'false');
  });
  ...
  it('does not set aria-describedby when no error', () => {
    render(<FloatingInput id="test-input" label="Vorname" />);
    expect(screen.getByLabelText(/Vorname/i)).not.toHaveAttribute('aria-describedby');
  });
  ...
  it('passes through additional html attributes', () => {
    render(<FloatingInput id="test-input" label="Vorname" type="email" maxLength={50} />);
