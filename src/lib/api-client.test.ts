import { getApiUrl, submitContactForm, API_ENDPOINTS, LEADS_ENDPOINTS } from './api-client';
...
describe('getApiUrl', () => {
  ...
  it('uses NEXT_PUBLIC_API_URL when defined', () => {
    ...
    expect(getApiUrl(API_ENDPOINTS.PROPERTIES)).toBe('https://api.example.com/api/properties');
  });

  it('falls back to localhost:3001 when env is not defined', () => {
    delete process.env.NEXT_PUBLIC_API_URL;
    expect(getApiUrl(LEADS_ENDPOINTS.CONTACT)).toBe('http://localhost:3001/api/v1/leads/contact');
  });
...
describe('submitContactForm', () => {
  ...
  it('throws ContactSubmitError when res.json() fails', async () => {
    ...
    await expect(submitContactForm(validPayload)).rejects.toBeInstanceOf(ContactSubmitError);
  });
});
