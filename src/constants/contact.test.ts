...
describe("CONTACT_FORM_LIMITS", () => {
  ...
  it("min values are smaller than their corresponding max values", () => {
    expect(CONTACT_FORM_LIMITS.FIRST_NAME_MIN).toBeLessThan(
      CONTACT_FORM_LIMITS.FIRST_NAME_MAX,
    );
    expect(CONTACT_FORM_LIMITS.LAST_NAME_MIN).toBeLessThan(
      CONTACT_FORM_LIMITS.LAST_NAME_MAX,
    );
    expect(CONTACT_FORM_LIMITS.MESSAGE_MIN).toBeLessThan(
      CONTACT_FORM_LIMITS.MESSAGE_MAX,
    );
  });
});
